...
A model-agnostic working prompt for decision making and risk management under uncertainty.
...
Applies to both phases. Classify each material uncertainty as one of:
...
Check each before any recommendation. For each trap below, respond with exactly one of three labels:
